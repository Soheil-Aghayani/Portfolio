...
| Back to OS Desktop | `ui/monitor-linear` | Uses a thin monitor outline instead of a filled return arrow | The destination is clearer and keeps the control aligned with the site-wide light icon weight. |
| Back navigation | `states/go-back` | Replaced the filled arrow with a 1.5px outline chevron | Back controls should remain legible without heavy visual mass. |
